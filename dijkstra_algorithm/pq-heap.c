...
_Bool pq_update(void *_pq, int label, int cost)
{
   ...
   if (
      ...
      int updated = pq->heapIDX[label];
      pq->cost[updated] = cost; // update the cost, but heap property is not satified 
       
      int daddy = GET_PARENT(updated);
 
      while(updated > 0 && pq->cost[updated] < pq->cost[daddy]){
         pq_swap(pq, daddy, updated);
     updated = daddy;
         daddy = GET_PARENT(updated);         
      }
      ret = true;
   }
   return ret;
